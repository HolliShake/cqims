...
import AcademicProgramModal from "@/views/pages/system/academic-programs/AcademicProgramModal.vue"
...
onMounted(async () => {
  const ID = helpers.security.decrypt(props.departmentid)

  console.log(">>", ID)

  acadStore.setDepartment(ID)
  acadStore.clear()

  loaded.value = false

  try {
    const { status: code, data: response } = await acadService.getAcademicProgramsByDepartmentId(ID)
      ...
      >
        ...
        <template #item.action="{ item }">
          <span class="text-no-wrap">
            <RouterLink
              :to="{ 
                name: 'system-schools-id-campuses-campusid-delivery-units-departmentid-academic-program-academicprogramid', 
                params: {
                  academicprogramid: helpers.security.encrypt(item.raw.id)
                },
                props: true,
              }"
            >
